// Users can click the button to open a ticket to apply for a second character, the corresponding permissions will be granted to the user and a messege will be sent in that new ticket
		...
		if ( interaction.isButton() ) {
			if (interaction.customId == "secondcharacter") {
                try {
                    ...
                    await interaction.editReply({
                        content: `An error has occurred, please try again later and if the problem persists contact a staff member. Code: Button 2nd application.`, ephemeral: true,
                    });
                    console.error(err);
                }
            }
